...
--       + v15 (gộp migration_cho_v14.sql — activation_token cho tài khoản
--         do cashier tạo trước cho khách)
--       + v16 (gộp V15_migration_add_unique_phone_app_user.sql — filtered
--         unique index cho phone; đồng thời chuyển email từ UNIQUE
--         constraint thường sang filtered unique index cho nhất quán,
--         tránh lặp lại đúng lỗi "chỉ cho phép 1 dòng NULL" đã gặp với
--         activation_token)
--
...
--
--  v16 – Gộp V15_migration_add_unique_phone_app_user.sql + fix email:
--       + app_user.phone: thêm filtered unique index UQ_app_user_phone
--         (WHERE phone IS NOT NULL) — tránh trùng SĐT đăng ký giữa các
--         AppUser, cùng pattern đã áp dụng cho activation_token
--       + app_user.email: bỏ UNIQUE constraint thường (inline), thay
--         bằng filtered unique index UQ_app_user_email — UNIQUE
--         constraint thường của SQL Server chỉ cho phép TỐI ĐA 1 dòng
--         NULL/bảng, sẽ vỡ ngay khi có user thứ 2 chưa nhập email
--         (VD: tài khoản cashier tạo trước cho khách tại quầy)
--
...
--         danh của GHN, khác hệ province/district/ward của provinces.open-api.vn
--         dùng cho dropdown hiện tại; giữ nguyên city/district cũ, NULL nếu địa
--         chỉ tạo trước khi có tính năng GHN, code fallback về region-based)
--       + address: + active (soft-delete/archive), mặc định 1 cho các địa chỉ có sẵn
--       + product: + weight (gram, bắt buộc cho GHN Calculate Fee), mặc định 300g
--         cho sản phẩm cũ, data mẫu điền theo loại sản phẩm thực tế
--       + Data mẫu: các địa chỉ Hà Nội/TP.HCM/Đà Nẵng có ghn_province_id/
--         ghn_district_id/ghn_ward_code mẫu (giá trị demo, cần thay bằng ID
--         thật lấy từ GHN Master Data API khi lên production)
--
...
-- activation_token/activation_expires_at (v15): dùng khi cashier tạo tài
-- khoản trước cho khách tại quầy — token NULL nếu tài khoản đã kích hoạt
-- (tự đăng ký online) hoặc đã hoàn tất kích hoạt qua link được gửi
...
-- FIX (v16): email trước đây dùng UNIQUE constraint thường (inline) — sẽ
-- vỡ ngay khi có user thứ 2 chưa nhập email, vì SQL Server chỉ cho phép
-- tối đa 1 dòng NULL/bảng với UNIQUE chuẩn. Chuyển sang filtered unique
-- index, cùng pattern với activation_token/phone.
CREATE UNIQUE INDEX UQ_app_user_email
    ON app_user(email)
    WHERE email IS NOT NULL;

-- FIX (v16, gộp V15_migration_add_unique_phone_app_user.sql): thêm
-- filtered unique index cho phone — tránh trùng SĐT đăng ký giữa các
-- AppUser, nhiều user chưa nhập SĐT (NULL) vẫn insert bình thường.
...
CREATE TABLE product (
    ...
    created_at  DATETIME      NOT NULL DEFAULT GETDATE(),
    weight      INT           NOT NULL DEFAULT 300  -- gram, dùng cho GHN Calculate Fee (v14)
);
...
-- Địa chỉ giao hàng
-- ghn_province_id/ghn_district_id/ghn_ward_code: mã định danh của GHN (v14),
--   khác hệ province/district/ward của provinces.open-api.vn dùng cho dropdown
--   hiện tại; giữ nguyên city/district (String) cũ để không vỡ dữ liệu đã có,
--   NULL nếu địa chỉ tạo trước khi có tính năng GHN (code fallback về region-based)
-- active: soft-delete/archive (v14) — xem AddressService.updateAddress()/deleteAddress()
...
-- id=9: khách được cashier1 tạo tài khoản trước tại quầy (v15), chưa
-- kích hoạt — password là placeholder ngẫu nhiên (không dùng được để login
-- cho tới khi khách bấm link kích hoạt và tự đặt mật khẩu), enabled=0
...
INSERT INTO app_user (username, password, email, phone, enabled, activation_token, activation_expires_at) VALUES
(N'vokhachf', N'{noop}TEMP-CHANGE-ME', N'vokhachf@gmail.com', N'0967890123', 0,
 N'a1b2c3d4-e5f6-47a8-9b01-c2d3e4f5a6b7', DATEADD(DAY, 7, SYSUTCDATETIME()));
GO
...
(9, 2);                                  -- vokhachf → ROLE_CUSTOMER (chờ kích hoạt, v15)
GO
...
-- ── PRODUCTS ───────────────────────────────────────────────
-- weight (gram): data mẫu ước lượng theo loại sản phẩm thực tế, dùng cho
-- GHN Calculate Fee (v14) — thay DEFAULT 300 chung chung
INSERT INTO product (name, description, price, category_id, weight) VALUES
...
-- ghn_province_id/ghn_district_id/ghn_ward_code: giá trị DEMO minh họa cho
-- luồng "đã có mã GHN" (v14) — cần thay bằng ID thật lấy từ GHN Master Data
-- API (/master-data/province|district|ward) khi triển khai thật.
-- active mặc định 1 (DEFAULT constraint), liệt kê tường minh cho rõ ràng.
INSERT INTO address (user_id, full_name, phone, street, city, district, country, is_default, ghn_province_id, ghn_district_id, ghn_ward_code, active) VALUES
(3, N'Nguyễn Văn A', N'0901234567', N'12 Nguyễn Trãi',    N'Hà Nội',  N'Thanh Xuân', N'Vietnam', 1, 201, 1542, N'1B2107', 1),
(3, N'Nguyễn Văn A', N'0901234567', N'45 Láng Hạ',         N'Hà Nội',  N'Đống Đa',    N'Vietnam', 0, 201, 1487, N'1A0113', 1),
(4, N'Trần Thị B',   N'0912345678', N'88 Lê Văn Việt',     N'TP.HCM',  N'Quận 9',     N'Vietnam', 1, 202, 3695, N'90737',  1),
(5, N'Lê Văn C',     N'0923456789', N'56 Trần Phú',        N'Đà Nẵng', N'Hải Châu',   N'Vietnam', 1, 205, 1490, N'20308',  1),
(6, N'Phạm Thị D',   N'0934567890', N'23 Nguyễn Văn Cừ',  N'TP.HCM',  N'Quận 5',     N'Vietnam', 1, 202, 3705, N'11109',  1),
(7, N'Hoàng Văn E',  N'0945678901', N'78 Đinh Tiên Hoàng', N'Hà Nội',  N'Hoàn Kiếm',  N'Vietnam', 1, 201, 1482, N'1A0107', 1);
GO
...
-- ghn_* để NULL để demo luồng fallback về region-based khi thiếu mã GHN
-- (khách vãng lai chưa từng chọn qua flow GHN dropdown)
INSERT INTO address (user_id, full_name, phone, street, city, district, country, is_default, active) VALUES
(NULL, N'Đỗ Văn Khách', N'0956789012', N'101 Cầu Giấy', N'Hà Nội', N'Cầu Giấy', N'Vietnam', 0, 1);
GO
