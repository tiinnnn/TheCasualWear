-- ============================================================
--  ClothingShop  –  Schema sạch + Seed Data hoàn chỉnh
--  Phiên bản: v4 – tất cả cột khai báo trong CREATE TABLE,
--              không dùng ALTER TABLE để thêm cột,
--              không dùng ALTER TABLE để thêm constraint sau
-- ============================================================
...
-- Roles
-- id: 1=ROLE_ADMIN | 2=ROLE_CUSTOMER | 3=ROLE_DELIVERY | 4=ROLE_OWNER
CREATE TABLE role (
...
-- Sản phẩm gốc (không có size/color/stock – quản lý ở product_variant)
...
-- Ảnh đại diện của sản phẩm (dùng chung cho mọi variant)
...
-- Ảnh riêng cho từng variant (màu cụ thể)
...
-- Mỗi đơn chỉ dùng 1 voucher; mỗi user chỉ dùng 1 voucher cùng loại
...
-- Sản phẩm trong giỏ hàng (không cần product_id vì variant đã biết product)
...
CREATE TABLE order_assignment (
    id           INT IDENTITY(1,1) PRIMARY KEY,
    order_id     INT          NOT NULL REFERENCES app_order(id),
    delivery_id  INT          NOT NULL REFERENCES app_user(id),
    assigned_by  INT               REFERENCES app_user(id),
...
-- Hồ sơ shipper (bao gồm khu vực phụ trách)
...
(1, 1), (1, 4),                -- owner  → ROLE_ADMIN + ROLE_OWNER
(2, 1),                        -- admin  → ROLE_ADMIN
(3, 3),                        -- delivery → ROLE_DELIVERY
(4, 2), (5, 2), (6, 2), (7, 2), (8, 2);  -- customers → ROLE_CUSTOMER
GO
...
--     7=SM Xanh Navy   | 8=Quần Jean      | 9=Quần Sweater | 10=Hoodie Đen
...
-- ── PRODUCT IMAGES ─────────────────────────────────────────
-- Mỗi sản phẩm 2 ảnh dùng chung cho mọi variant
...
-- ── VARIANT IMAGES ─────────────────────────────────────────
-- sort_order: 1=ảnh chính, 2=ảnh phụ
...
-- ── ADDRESSES ──────────────────────────────────────────────
-- user 4=nguyenvana | 5=tranthib | 6=levanc | 7=phamthid | 8=hoangvane
...
-- ── ORDERS ─────────────────────────────────────────────────
-- nguyenvana (user 4) – 4 đơn: id 1-4
...
-- ── ORDER VOUCHERS ──────────────────────────────────────────
-- Đơn 1 (nguyenvana): total=747k, WELCOME10 giảm 10% → min(74700, 50000) = 50,000đ
-- Đơn 5 (tranthib):   total=799k, SUMMER20 giảm 20%  → min(159800, 100000) = 100,000đ
...
-- ── DELIVERY PROFILE ───────────────────────────────────────
INSERT INTO delivery_profile (user_id, area, is_available) VALUES
(3, N'Hà Nội – Hoàn Kiếm, Đống Đa', 1);
GO

-- ── ORDER ASSIGNMENT ───────────────────────────────────────
-- Phân công đơn SHIPPING (id=3,11) và DELIVERED (id=9) cho shipper (user 3)
INSERT INTO order_assignment (order_id, delivery_id, assigned_by, status, picked_up_at, delivered_at) VALUES
(3,  3, 2, N'ASSIGNED',  NULL,                              NULL),
(9,  3, 2, N'DELIVERED', DATEADD(DAY,-2,GETDATE()),         DATEADD(DAY,-1,GETDATE())),
(11, 3, 2, N'ASSIGNED',  NULL,                              NULL);
GO

-- ── WISHLISTS ──────────────────────────────────────────────
INSERT INTO wishlist (user_id, product_id) VALUES
(4, 5),   -- nguyenvana yêu thích SM Trắng
(4, 10),  -- nguyenvana yêu thích Hoodie Đen
(5, 8),   -- tranthib yêu thích Quần Jean
(5, 3),   -- tranthib yêu thích AT In Mèo Trắng
(6, 10),  -- levanc yêu thích Hoodie Đen
(7, 7),   -- phamthid yêu thích SM Xanh Navy
(8, 9);   -- hoangvane yêu thích Quần Sweater
GO

-- ── NOTIFICATIONS ──────────────────────────────────────────
INSERT INTO notification (user_id, message, link, is_read) VALUES
(4, N'Đơn hàng #1 của bạn đã hoàn thành.',              N'/orders/1',  1),
(4, N'Đơn hàng #3 của bạn đang trên đường giao.',        N'/orders/3',  0),
(5, N'Đơn hàng #5 của bạn đã hoàn thành.',              N'/orders/5',  1),
(5, N'Đơn hàng #6 của bạn đã được xác nhận.',           N'/orders/6',  0),
(6, N'Đơn hàng #9 của bạn đã được giao thành công.',    N'/orders/9',  0),
(7, N'Đơn hàng #11 của bạn đang trên đường giao.',      N'/orders/11', 0),
(8, N'Đơn hàng #13 của bạn đã bị huỷ.',                 N'/orders/13', 1);
GO
